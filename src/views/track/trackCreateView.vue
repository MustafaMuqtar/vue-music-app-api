<template>
  <div class="container bg-dark text-white">
    <form enctype="multipart/form-data " @submit.prevent>
        ...
        <div class="dropdown">
          ...
          <ul class="dropdown-menu">
            <li class="dropdown-item" id="dropdown" @click="trackService.dropdownCh(artist.id)"
              v-for="(artist, index) in artistService.artists" :key="index">
              <span>{{ index + 1 }}</span>. {{ artist.fullName }}
            </li>
          </ul>
        </div>
      ...
      <button type="submit" @click="trackService.addTrack()" class="btn btn-primary">
        <span v-if="trackService.isAdding"><span class="spinner-grow spinner-grow-sm" role="status"
